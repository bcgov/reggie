  ...
  for (var k in phases) {
    if (phases.hasOwnProperty(k)) {
      ...
      if (k == build_phase) {
        ...
        oc.raw('get', ['bc'], {selector:`app-name=${phase.name},env-id=${phase.changeId},env-name!=prod,!shared`, namespace:phase.namespace, 'output':'custom-columns=kind:.spec.output.to.kind,name:.spec.output.to.name', 'no-headers':'true'})
